...
/*   Updated: 2022/02/13 13:05:11 by lpaulo-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const double redWeight = 0.7;
const double greenWeight = 0.5;
...
void render_mandelbrot(void)
{
	...
	while (x--)
	{
		...
		while (y--)
		{
			...
			normalized_iterations = map_d(mdlbtAtXY.iterations, 0, max_iterations, 0, 1);
			brightness = map_d(sqrt(normalized_iterations), 0, 1, 0, 255 * 3);

			const int blue = brightness * blueWeight;
			brightness -= 255;
			const int red = brightness * redWeight;
			brightness -= 255;
			const int green = brightness * greenWeight;
